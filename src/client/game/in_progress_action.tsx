...
export const InProgressData = createContext<undefined | InProgressAction>(
  undefined,
);
const SetInProgressData = createContext<
  ((data: InProgressAction) => void) | undefined
>(undefined);

export function InProgressActionProvider({
  children,
}: {
  children?: ReactNode;
}) {
  ...
  const externalSetData = useCallback(
    (data: InProgressAction) => setData(freeze(data)),
    [setData],
  );
  return (
    <InProgressData.Provider value={data}>
