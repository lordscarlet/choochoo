...
const AwaitingPlayerContext = createContext<(() => void) | undefined>(undefined);
...
export function AwaitingContextProvider({ children }: { children: ReactNode }) {
  const [numAwaiting, incrNumAwaiting] = useReducer((prev: number, incr: number) => prev + incr, 0);
  ...
  return <AwaitingPlayerContext.Provider value={increment}>
