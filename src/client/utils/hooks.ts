...
export function useTypedCallback<Args extends Tuple, Deps extends Tuple, T>(
  cb: (...deps: Deps) => (...args: Args) => T,
  deps: NoInfer<Deps>,
): (...args: Args) => T {
  return useCallback(cb(...deps), deps);
}

export function useTypedMemo<Deps extends Tuple, T>(
  cb: (...deps: Deps) => T,
  deps: NoInfer<Deps>,
): T {
...
export function useMostRecentValue<T>(
  ...
  const result = useMemo(() => {
    const changedValueIndex = values.findIndex(
      (value, index) => value !== previousValues.current[index],
    );
    return changedValueIndex !== -1
      ? values[changedValueIndex]
