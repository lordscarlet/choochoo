...
export function HexGrid({ onClick, onClickInterCity, rotation, fullMapVersion, highlightedTrack, highlightedConnections, selectedGood, grid, clickTargets, gameKey }: HexGridProps) {
  ...
  const [internalViewBox, setViewBox] = useState({ x: 0, y: 0, width: 0, height: 0 });
  ...
  return useMemo(() => {
    return <>
      {...iterate(grid.bottomRight.col - grid.topLeft.col + 1, (index) =>
        <DoubleHeightNumber
          index={index} rotation={rotation} origin={origin} spacing={spacing} size={size} isRow={false} />)}
      {...iterate(grid.bottomRight.row - grid.topLeft.row + 1, (index) =>
        <DoubleHeightNumber
          index={index} rotation={rotation} origin={origin} spacing={spacing} size={size} isRow={true} />)}
    </>;
  }, [grid.topLeft.col, grid.topLeft.row, grid.bottomRight.col, grid.bottomRight.row, rotation, size, origin]);
}
...
function DoubleHeightNumber({ index, rotation, origin, spacing, size, isRow }: DoubleHeightNumberProps) {
  ...
  const x = rotation != null ?
    (isRow ? yTransform + origin.x : xBasis) :
    (isRow ? xBasis : xTransform + origin.x);

  const y = rotation != null ?
    (isRow ? yBasis : xTransform + origin.y) :
    (isRow ? yTransform + origin.y : yBasis);

  const content = isRow ? doubleHeight.toRowString() : doubleHeight.toColString();
