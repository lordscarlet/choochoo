...
export const Path = z.object({
  owner: z.nativeEnum(PlayerColor),
  endingStop: z.object({q: z.number(), r: z.number()}),
});
...
export const MoveData = z.object({
  ...
  path: z.array(Path),
  startingCity: z.object({q: z.number(), r: z.number()}),
...
export class MoveAction implements ActionProcessor<MoveData> {
  ...
  validate(action: MoveData): void {
    ...
    const startingCity = this.grid.lookup(Coordinates.from(action.startingCity));
    ...
    const endingLocation = this.grid.lookup(Coordinates.from(peek(action.path).endingStop));
    ...
    for (const step of action.path.slice(0, action.path.length - 1)) {
      const location = this.grid.lookup(Coordinates.from(step.endingStop));
    ...
    for (const step of action.path) {
      const toCoordinates = Coordinates.from(step.endingStop);
      ...
      const ownedByPlayer = eligibleOwners.filter((eligibleOwners) => eligibleOwners === step.owner);
