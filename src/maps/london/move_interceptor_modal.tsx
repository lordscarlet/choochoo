...
import {useCurrentPlayer, useGrid} from "../../client/utils/injection_context";
import {
  Button,
  DropdownProps,
  Header,
...
export function LondonMoveInterceptorModal({
  ...
  const grid = useGrid();
  const { emit: emitMoveAction, isPending: isPending } = useAction(LondonMoveAction);
  ...
  const options = useMemo(() => {
    ...
    let result: Array<{key: string, text: string, value: string}> = [];

    const start = grid.get(moveData.startingCity);
    if (start && start instanceof City) {
      result.push({key: start.name(), text: start.name(), value: start.name()});
    }
  ...
  return (
    <Modal closeIcon open={moveData != null} onClose={clearMoveData}>
      ...
      <ModalContent>
        <p>Select a city to which a good should be added for instant production.</p>
        <p>The top good from the goods display will be moved to the city. If there are no goods left, a random cube will be drawn from the bag and added to the goods display. This will make the action non-reversible.</p>
        <Select
          placeholder="City"
          options={options}
          onChange={onChange}
